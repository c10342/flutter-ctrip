...
class TravelItemModel {
  ...
  TravelItemModel({required this.totalCount, required this.resultList});

  factory TravelItemModel.fromJson(Map<String, dynamic> json) {
    List<TravelItem> list = [];
    if (json['resultList'] != null) {
      json['resultList'].forEach((v) {
        list.add(new TravelItem.fromJson(v));
      });
    }

    return TravelItemModel(
        totalCount: json['totalCount'] ?? 0, resultList: list);
  }
...
class TravelItem {
  ...
  TravelItem({required this.type,  this.article});
